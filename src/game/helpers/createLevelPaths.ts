export function createLevelPaths(width: number, height: number): Map<string, Phaser.Curves.Path> {
    ...
    const zigZag = new Phaser.Curves.Path(400, -50).lineTo(400, 100).lineTo(300, 150).lineTo(500, 200)
    paths.set('path_zigzag', zigZag)

    const bossEntry = new Phaser.Curves.Path(width / 2, -150).lineTo(width / 2, 150)
